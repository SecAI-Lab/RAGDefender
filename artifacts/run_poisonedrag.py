...
def get_log_name(test_params):
    ...
    os.makedirs(f"logs/main_logs_PoisonedRAG_{str(test_params['seed'])}", exist_ok=True)
    ...
    return f"logs/main_logs_PoisonedRAG_{str(test_params['seed'])}/{log_name}_{test_params['additional_adv_per_query']}.txt", log_name
...
test_params = {
    ...
    'query_results_dir': 'main_PoisonedRAG',
...
for seed in [12]:
    ...
    test_params['query_results_dir'] = f"main_PoisonedRAG_{str(seed)}"
    # for eval_code in ["dpr", "ance"]:
