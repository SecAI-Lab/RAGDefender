...
def top_similar_pairs(texts, model, top_k):
    """Calculates and returns the top_k most similar pairs of texts along with their cosine similarity."""

    embeddings = model.encode(texts, convert_to_tensor=True)
    cos_similarities = sentence_transformers_util.cos_sim(embeddings, embeddings)

    # Get unique pairs with their similarity scores
    pairs = []
    for i in range(len(texts)):
        for j in range(i + 1, len(texts)):
            pairs.append((i, j, cos_similarities[i][j].item()))

    # Sort pairs by similarity in descending order and get the top_k
...
def load_beir_datasets(dataset_name, split):
    assert dataset_name in ['nq', 'msmarco', 'hotpotqa', 'fiqa', 'quora']
...
def load_beir_datasets_md(dataset_name, split):
...
def load_beir_datasets_md2(dataset_name, split):
    assert dataset_name in ['nq', 'msmarco', 'hotpotqa']
    #if dataset_name == 'msmarco': split = 'train'
    url = "https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/{}.zip".format(dataset_name)
    out_dir = "datasets"
...
def save_results_safe(results, dir, file_name="debug"):
    json_dict = json.dumps(results, cls=NpEncoder)
    dict_from_str = json.loads(json_dict)
    if not os.path.exists(f'results/query_results/{dir}'):
        os.makedirs(f'results/query_results/{dir}', exist_ok=True)
    with open(os.path.join(f'results/query_results/{dir}', f'{file_name}safe.json'), 'w', encoding='utf-8') as f:
        json.dump(dict_from_str, f)
